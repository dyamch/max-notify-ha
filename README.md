...
- Поддерживаются **текстовые сообщения**, а также **фото и файлы** через сервисы `max_notify.send_photo` и `max_notify.send_document` (ограничения и форматы задаёт сторона [notify.a161.ru](https://notify.a161.ru/), обычно до **4 МБ** на вложение).
